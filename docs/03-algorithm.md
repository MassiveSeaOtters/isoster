...
- Eccentric-anomaly mode (`use_eccentric_anomaly=True`): harmonics use `psi`. The geometry update consumes those `psi`-basis coefficients **directly** — there is no conversion back to `phi`. `phi` is carried alongside (and sigma-clipped in lockstep) so the angular bookkeeping stays aligned for diagnostics and error propagation, but it does not enter the update. Consuming the `psi` coefficients directly is **correct, not an approximation**: the Jedrzejewski geometry corrections are themselves derived from an eccentric-anomaly parametrisation of the trial ellipse. A major-axis offset `d` puts a pure `cos(psi)` term of amplitude `-d*grad` into the sampled ring; a minor-axis offset a pure `sin(psi)` term with an extra `1/(1-eps)`; an ellipticity error a pure `cos(2*psi)` term with `a*grad/(2*(1-eps))`; a PA error a pure `sin(2*psi)` term. Inverting those four relations reproduces exactly the `(1-eps)`, `2*(1-eps)/sma` and `((1-eps)**2 - 1)` factors in the update code. It is the default `phi` mode that feeds the formulas coefficients from the wrong basis, for IRAF/photutils compatibility; the error vanishes as `eps -> 0`.

  Do **not** justify the `phi` mode with a fixed-point argument ("the iteration drives the residual to zero, so the basis cannot matter"). The map `tan(phi) = (1 - eps) * tan(psi)` is nonlinear and does not preserve which Fourier orders vanish: a pure `cos(3*psi)` residual at `b/a = 0.3` has exactly zero first- and second-order content in `psi` but a first-order cosine coefficient of `-0.180` in `phi`. The two bases place the fixed point in different locations, so real higher-order structure leaks into the geometry update in `phi` mode.

  Earlier versions of this line, and of the comments in `fitting.py`, claimed geometry updates used `phi`; that was never what the code did (corrected in `80618bb`).
