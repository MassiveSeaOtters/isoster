...
from benchmarks.timing.accuracy_thresholds import CONTAMINATION  # noqa: E402
...
DISQUALIFYING_PROCESSES = ("claude", "pytest", "uv run")
...
def competing_processes() -> List[str]:
    ...
    for line in output.splitlines()[1:]:
        ...
        if share_value < 1.0:
            continue
        if any(name in command for name in DISQUALIFYING_PROCESSES):
...
def evaluate(result: Dict[str, object], processes: List[str], thermal: Dict[str, object]) -> List[str]:
    problems = []
...
def main() -> None:
    ...
    problems = evaluate(result, processes, thermal)
    ...
    if args.json:
        args.json.write_text(json.dumps({**result, "processes": processes, "thermal": thermal}, indent=2))
