...
def fit_first_and_second_harmonics(phi, intensity):
    ...
    if len(phi) < 5:
        return np.zeros(5), None # Not enough points
        
    # coeffs, residuals, rank, s = np.linalg.lstsq(A, intensity, rcond=None)
    # To get covariance, we need (A.T A)^-1
    # We can compute this explicitly for small matrices (5x5)
    ...
    # 3. Solve for coeffs: ATA @ x = ATy
    try:
        # Use solve instead of inv for stability
        coeffs = np.linalg.solve(ATA, ATy)
        
        # 4. Compute inverse of ATA for covariance
        # Since ATA is symmetric positive definite (usually), we can use inv
...
def compute_parameter_errors(phi, intens, x0, y0, sma, eps, pa, gradient, cov_matrix=None):
    ...
    cov_matrix : 2D array, optional
        Covariance matrix (inverse Hessian) from the harmonic fit.
        If provided, avoids re-fitting.
    ...
    try:
        if cov_matrix is None:
            # Fallback: re-fit if no covariance provided (shouldn't happen with optimized flow)
            # This path is now deprecated for performance but kept for safety
            from scipy.optimize import leastsq
            ...
            # FAST PATH: Use pre-computed covariance from linear fit
            # We need to compute residual variance to scale it
            # Re-evaluate model to get residuals
            # Note: We don't have coeffs here, but we can get residuals from intens directly?
            # Actually, we need the model values.
            # Let's assume the caller passes the RMS or we re-compute coeffs quickly.
            # Better: just re-compute coeffs using linear algebra (fast)
            ...
            # Scale covariance (ATA^-1 * sigma^2)
            covariance = cov_matrix * var_residual
            errors = np.sqrt(np.diagonal(covariance))
...
def compute_gradient(image, mask, x0, y0, sma, eps, pa, step=0.1, linear_growth=False, previous_gradient=None, current_data=None):
    ...
    ----------
    ...
    current_data : tuple (phi, intens), optional
        Pre-extracted data at current SMA to avoid re-extraction.
    """
    # Extract data at current SMA (or reuse)
...
def fit_isophote(image, mask, sma, start_geometry, config, going_inwards=False):
    ...
    This function performs the iterative least-squares fitting of harmonic functions 
    to the intensity samples extracted along an elliptical path. It updates the 
    geometry (center, ellipticity, position angle) based on the harmonic coefficients 
    ...
    ALGORITHM:
    1. Extract intensity samples along the current elliptical path.
    2. Apply sigma clipping to remove outliers (e.g., stars, bad pixels).
    3. Fit 1st and 2nd order harmonics to the intensity distribution.
    4. Compute geometric corrections (delta x0, y0, eps, pa) from harmonic amplitudes.
    ...
    mask : 2D boolean array or None
        Combined mask (True = bad pixel).
    sma : float
        Target semi-major axis length (pixels).
    start_geometry : dict
        Initial guess for geometry {'x0', 'y0', 'eps', 'pa'}.
    config : dict
        Configuration dictionary containing:
        - 'maxit': Max iterations (default: 50)
        - 'conver': Convergence threshold (default: 0.05)
        - 'sclip': Sigma clipping threshold (default: 3.0)
        - 'nclip': Number of clipping iterations (default: 0)
        - 'fflag': Max fraction of flagged data (default: 0.5)
        - 'maxgerr': Max relative gradient error (default: 0.5)
        - 'fix_center', 'fix_eps', 'fix_pa': Boolean flags to fix parameters.
    going_inwards : bool
        If True, the fitting sequence is moving inwards (sma decreasing).
        This affects some quality control logic (e.g. maxgerr check).

    Returns
    -------
    result : dict
        Dictionary containing the fitted geometry and status:
        - 'sma', 'x0', 'y0', 'eps', 'pa': Fitted parameters.
        - 'intens', 'rms': Mean intensity and RMS at this SMA.
        - 'intens_err', 'x0_err', 'y0_err', 'eps_err', 'pa_err': Errors.
        - 'stop_code': Status code (0=success, 1=fflag, 2=maxit, -1=fail).
    ...
    debug = config.get('debug', False)    # Debug mode
    ...
    for i in range(maxit):
        ...
        # 2. Fit harmonics (and get covariance)
        coeffs, cov_matrix = fit_first_and_second_harmonics(phi, intens)
        y0_fit, A1, B1, A2, B2 = coeffs
        
        # 3. COMPUTE GRADIENT (reuse current data)
        gradient, gradient_error = compute_gradient(
            image, mask, x0, y0, sma, eps, pa, astep, linear_growth, previous_gradient,
        ...
        if abs(max_amp) < min_amplitude:
            ...
            if compute_errors:
                x0_err, y0_err, eps_err, pa_err = compute_parameter_errors(
                    phi, intens, x0, y0, sma, eps, pa, gradient, cov_matrix
                )
            else:
...
def fit_central_pixel(image, mask, x0, y0, debug=False):
    ...
    x0, y0 : float
    debug : bool
        If True, return additional debug information.
        
    Returns
    -------
    result : dict
        Fitted geometry and status.
        If debug is True, includes 'ndata', 'nflag', 'grad', 'grad_error', 'grad_r_error'.
    """
...
def fit_image(image, mask, config):
    ...
    -------
    result : dict
        A dictionary containing the fitting results and configuration.
        ...
        Each dictionary in the 'isophotes' list represents one fitted isophote 
        and contains the following keys:
        
        Geometry:
        - 'sma': Semi-major axis length (pixels).
        - 'x0', 'y0': Center coordinates (pixels).
        - 'eps': Ellipticity (1 - b/a).
        - 'pa': Position angle (radians, counter-clockwise from positive x-axis).
        
        Photometry:
        - 'intens': Mean intensity along the isophote.
        - 'rms': RMS of intensity fluctuations along the isophote.
        - 'intens_err': Error in the mean intensity (rms / sqrt(N)).
        
        Errors (if compute_errors=True):
        - 'x0_err', 'y0_err': Errors in center coordinates.
        - 'eps_err': Error in ellipticity.
        - 'pa_err': Error in position angle.
        
        Shape Deviations (if compute_deviations=True):
        - 'a3', 'b3': 3rd order harmonic amplitudes (normalized).
        - 'a4', 'b4': 4th order harmonic amplitudes (normalized).
        - 'a3_err', 'b3_err', 'a4_err', 'b4_err': Errors in harmonic amplitudes.
        
        Status:
        - 'stop_code': Integer status code.
            0: Normal convergence.
            1: Too many flagged points (fflag).
            2: Max iterations reached.
            3: Too few points.
            -1: Failed/Diverged.
        - 'niter': Number of iterations performed.
        - 'valid': Boolean indicating if the fit is considered valid.
        
        Debug Information (only if config['debug'] is True):
        - 'ndata': Number of valid data points used.
        - 'nflag': Number of flagged/clipped data points.
        - 'grad': Radial intensity gradient.
        - 'grad_error': Error in radial gradient.
        - 'grad_r_error': Relative error in radial gradient.
    """
    # Initial parameters
    ...
    if minsma <= 0.0:
        ...
        res = fit_central_pixel(image, mask, cx, cy, debug=config.get('debug', False))
        inwards_results.append(res)
            
    # Combine results
    # inwards_results are in decreasing order of SMA. Reverse them.
    all_isophotes = inwards_results[::-1] + results
    
    return {
        'isophotes': all_isophotes,
        'config': config
    }
